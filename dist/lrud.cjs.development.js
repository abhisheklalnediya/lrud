    ...
    if (nodeDefinition.onMountAssignFocusTo !== undefined) {
      {
        if (onMountAssignFocusTo !== null) {
           warning('[Focus]: More than one onMountAssignFocusTo was encountered while creating a new focus subtree. This may represent an error in your code. ' + 'We strongly encourage you to ensure that only a single node is assigned an onMountAssignFocusTo when creating a focus subtree. ' + 'Your onMountAssignFocusTo has been ignored, and the first child will be assigned focus.', 'MORE_THAN_ONE_ONMOUNTFOCUS') ;
        }
      }
    ...
    if (!isCreatingNewNode) {
...

      {
        if (isFinalNode && setFocusGoal && !focusState.nodes[setFocusGoal]) {
           warning('You configured an onMountAssignFocusTo that was not found in the focus tree. This may represent an error in your application. ' + 'Please make sure that the node specified by onMountAssignFocusTo is created at the same time as the parent.', 'NOT_FOUND_ON_MOUNT_FOCUS') ;
        }
      }
...
function computeFocusHierarchy(_ref2) {
  ...
  if (explicitlyAssignFocus) {
    ...
    {
      if (!assignedNode) {
         warning('You attempted to explicitly focus a node that was not found in the focus tree. ' + 'This may represent a bug in your application. ' + 'You should ensure that a node that matches onMountAssignFocusTo is created and not disabled. ' + 'This onMountAssignFocusTo value has been ignored; focus will be computed automatically.', 'EXPLICIT_FOCUS_ERROR') ;
      }
    }
...
function deleteNode(_ref) {
  ...
  if (nodeId === 'root') {
    {
       warning('You attempted to delete the root node. ' + 'The root node of a focus tree cannot be deleted. ' + 'The focus tree has not been changed;', 'ATTEMPTED_TO_DELETE_ROOT') ;
    }
...
function createFocusStore(_temp) {
    ...
    if (!currentNode) {
      {
        if (typeof nodeId !== 'string') {
           warning("You called setFocus with a node ID that is not a string. The node ID that you passed was: " + nodeId + ". All node IDs are strings.", 'NODE_ID_NOT_STRING_TO_SET_FOCUS') ;
        } else {
           warning('You attempted to set focus to a node that does not exist in the focus tree.', 'NODE_DOES_NOT_EXIST') ;
        }
      }
      ...
      {
         warning('You attempted to set focus to a node that is exiting. This has no effect, but it may represent a memory leak in your application.', 'FOCUS_SET_TO_EXITING_NODE') ;
      }
  ...
  function updateNode(nodeId, update) {
    ...
    if (!currentNode) {
      {
        if (update.disabled) {
           warning('You attempted to disable a node that does not exist in the focus tree. ' + 'This has no effect, but it may represent an error in your code.', 'DISABLE_NODE_THAT_DOES_NOT_EXIST') ;
        }

        if (update.isExiting) {
           warning('You attempted to exit a node that does not exist in the focus tree. ' + 'This has no effect, but it may represent an error in your code.', 'EXIT_NODE_THAT_DOES_NOT_EXIST') ;
        }
      }
    ...
    if (nodeId === 'root') {
      {
        if (update.disabled) {
           warning('You attempted to disable the root node. ' + 'The root node of a focus tree cannot be disabled. ' + 'This has no effect, but it may represent an error in your code.', 'DISABLE_ROOT_NODE') ;
        }

        if (update.isExiting) {
           warning('You attempted to exit the root node. ' + 'The root node of a focus tree cannot be exited. ' + 'This has no effect, but it may represent an error in your code.', 'EXIT_ROOT_NODE') ;
        }
      }
...
var FocusContext = /*#__PURE__*/React__default.createContext(null);
  ...
  var _useState = React.useState(function () {
    {
      if (orientation !== undefined && orientation !== 'vertical' && orientation !== 'horizontal') {
         warning('An invalid orientation was passed to the FocusRoot. The orientation must either be "vertical" or "horizontal."', 'INVALID_ROOT_ORIENTATION') ;
      }
    }
...
function FocusNode(_ref2, ref) {
  ...
  var elRef = React.useRef(null); // We store the callbacks in a ref so that we can pass a wrapper function into the underlying
  ...
  React.useImperativeHandle(ref, // I may need to update this based on this comment to make TS happy:
  ...
  var _useState = React.useState(function () {
    ...
    {
      if (reservedFocusId) {
         warning('A focus node with an invalid focus ID was created: "root". This is a reserved ID, so it has been ' + 'ignored. Please choose another ID if you wish to specify an ID.', 'ROOT_ID_WAS_PASSED') ;
      }

      if (nonStringFocusId || emptyStringNode) {
         warning('A focus node with an invalid (non-string or empty string) focus ID was created. This is a not supported ID type (expected non-empty string), so it has been ' + 'ignored. Please choose another ID if you wish to specify an ID.', 'INVALID_FOCUS_ID_PASSED') ;
      }
    }
  ...
  var contextValue = React.useContext(FocusContext$1.Context);
  ...
  var _useState2 = React.useState(function () {
...

    {
      if (isGrid && orientation) {
         warning('You passed the orientation prop to a grid focus node. ' + 'This prop has no effect on grid nodes, but it may represent an error in your code. ' + ("This node has a focus ID of " + nodeId + "."), 'ORIENTATION_ON_GRID') ;
      }

      if (isGrid && defaultFocusChild) {
         warning('You passed the defaultFocusChild prop to a grid focus node. ' + 'This prop has no effect on grid nodes, but it may represent an error in your code. ' + ("This node has a focus ID of " + nodeId + "."), 'PREFERRED_CHILD_INDEX_ON_GRID') ;
      }

      if (onGridMove && !isGrid) {
         warning('You passed the onGridMove prop to a node that is not a grid. ' + 'This will have no effect, but it may represent an error in your code. ' + ("This node has a focus ID of " + nodeId + "."), 'GRID_MOVE_NOT_ON_GRID') ;
      } else if (onMove && isGrid) {
         warning('You passed the onMove prop to a grid Focus Node. ' + 'onMove does not work on grid nodes. Did you mean to pass onGridMove instead? ' + ("This node has a focus ID of " + nodeId + "."), 'ON_MOVE_ON_GRID') ;
      }

      if (forgetTrapFocusHierarchy && !nodeDefinition.trap) {
         warning('You passed the forgetTrapFocusHierarchy prop to a focus node that is not a trap. ' + 'This will have no effect, but it may represent an error in your code. ' + ("This node has a focus ID of " + nodeId + "."), 'RESTORE_TRAP_FOCUS_WITHOUT_TRAP') ;
      }
    }

    if (!contextValue) {
      {
         warning('A FocusProvider was not found in the tree. Did you forget to mount it?', 'NO_FOCUS_PROVIDER_DETECTED') ;
      }
  ...
  var nodeExistsInTree = React.useRef(false);
  var dynamicProps = React.useMemo(function () {
    return {
  ...
  return React__default.createElement(FocusContext$1.Context.Provider, {
    value: staticDefinitions.providerValue
  }, React.createElement(elementType, _extends({}, otherProps, computedProps, {
    ref: elRef,
...
var ForwardedFocusNode = /*#__PURE__*/React.forwardRef(FocusNode);
...
function useFocusNodeById(focusId) {
    ...
    if (!contextValue) {
    ...
    } else {
      {
        if (typeof focusId !== 'string') {
           warning("You passed a non-string focus ID to useFocusNodeById: " + focusId + ". Focus IDs are always strings. " + 'This may represent an error in your code.', 'FOCUS_ID_NOT_STRING') ;
        }
      }
  ...
  var focusNodeRef = React.useRef(focusNode);
  ...
  React.useEffect(checkForSync, [focusId]);
  React.useEffect(function () {
    if (!contextValue) {
      return;
    }
...
function useFocusNode(nodeId) {
  React.useEffect(function () {
     warning("You used the hook `useFocusNode`. This hook has been renamed to `useFocusNodeById`. Please refactor your code to use the new hook instead.", 'USE_FOCUS_NODE_RENAMED') ;
  }, []);
...
function useFocusHierarchy() {
  ...
  var focusHierarchyRef = React.useRef(focusHierarchy);
...
function useLeafFocusedNode() {
  ...
  var leafId = React.useMemo(function () {
    var _focusHierarchy;
...
function useFocusEvents(nodeId, events) {
  ...
  var _useState = React.useState(nodeId),
  ...
  useChange(nodeId, function (currentId, prevId) {
    ...
    {
      if (currentId !== prevId) {
         warning("The nodeId passed into useFocusEvents changed. This change has been ignored: the nodeId cannot be changed.", 'FOCUS_EVENTS_NODE_ID_CHANGED') ;
      }
    }
  });
...
exports.FocusNode = ForwardedFocusNode;
exports.FocusRoot = FocusRoot$1;
exports.useActiveNode = useActiveNode;
exports.useFocusEvents = useFocusEvents;
exports.useFocusHierarchy = useFocusHierarchy;
exports.useFocusNode = useFocusNode;
exports.useFocusNodeById = useFocusNodeById;
exports.useFocusStoreDangerously = useFocusStoreDangerously;
exports.useLeafFocusedNode = useLeafFocusedNode;
exports.useProcessKey = useProcessKey;
exports.useSetFocus = useSetFocus;
//# sourceMappingURL=lrud.cjs.development.js.map
